...
public class Spider extends Crawler {
	...
	public void onFinish() {
		try {
			...
			System.out.println("Spider " + this.threadId + " executed in " + this.benchmark + " seconds and found " + this.foundLinks + " new links.");
	...
	public ArrayList<String> provider() {
		...
		try {
			...
			if (size == 0) {
				...
				PreparedStatement select = conn.prepareStatement("SELECT id_page, url FROM page WHERE fk_store = ? AND was_scanned = 0 AND in_use = 0 AND is_product = 0 LIMIT 0, 10");
